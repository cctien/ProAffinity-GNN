        ...
        with open(filename, "r") as f:
            ...
            for chain in chain_list:
                ...
                for line in lines:
                    ...
                    if (
                        current_res != previous_res and previous_res != -1
                    ):  # this is a new residue, append the previous one
            ...
            for atomA in resA["atoms"]:
                ...
                for atomB in resB["atoms"]:
                    ...
                    if distance <= dis_thred:
                        ...
                        c_alpha_dist = math.sqrt(
                            (ca1_x - ca2_x) ** 2 + (ca1_y - ca2_y) ** 2 + (ca1_z - ca2_z) ** 2
                        )
                        # print(c_alpha_dist)
...
class GraphNetwork(torch.nn.Module):
    def __init__(
        ...
        self.graph1 = AttentiveFPModel(
            in_channels, hidden_channels, out_channels, edge_dim, num_layers, num_timesteps, dropout
        )
        self.graph2 = AttentiveFPModel(
            in_channels, hidden_channels, out_channels, edge_dim, num_layers, num_timesteps, dropout
        )
        self.graph3 = AttentiveFPModel(
            in_channels, hidden_channels, out_channels, edge_dim, num_layers, num_timesteps, dropout
        )
...
with torch.no_grad():
    ...
    for batch_inter, batch_intra1, batch_intra2 in zip(
        test_loader_inter, test_loader_intra1, test_loader_intra2
    ):
        ...
        if isinstance(batch_intra2.edge_attr, list):
            batch_intra2.edge_attr = torch.stack(batch_intra2.edge_attr, dim=0).to(
                device
